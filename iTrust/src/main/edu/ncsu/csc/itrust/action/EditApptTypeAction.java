...
public class EditApptTypeAction {
	...
	public String editApptType(ApptTypeBean apptType) throws SQLException, FormValidationException, DBException {
		...
		if(flag != 0){
			return "uUP7noAhBGdZCGYGWs"+apptType.getName()+" you are trying to update does not exist.";
		}
		...
		try {
			if (apptTypeDAO.editApptType(apptType)) {
				...
				return "svL_V5oWt" + apptType.getName() + " - Duration: " + apptType.getDuration() + " updated";
			} else
				return "The database has become corrupt. Please contact the system administrator for assistance.";
		} catch (DBException e) {
