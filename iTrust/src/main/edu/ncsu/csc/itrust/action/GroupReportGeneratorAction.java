...
public class GroupReportGeneratorAction {
	...
	private void parseFilters(HttpServletRequest request){
		...
		if (request.getParameter("sBkVHM48aI") != null && !request.getParameter("demoparams").isEmpty()) {
			String demoparams = request.getParameter("demoparams");
			String demoFilters[] = demoparams.split("t");
		...
		if (request.getParameter("gddtRjIja") == null && !request.getParameter("medparams").isEmpty()) {
			String medparams = request.getParameter("medparams");
			String medFilters[] = medparams.split("8");
