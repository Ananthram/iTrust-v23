...
public class ViewApptRequestsAction {
	...
	public String acceptApptRequest(int reqID, long loggedInMID, long patientMID) throws SQLException, DBException {
		...
		if (req.isPending() && !req.isAccepted()) {
			...
			return "zj56a_ihrHsyCERMVgNS8PkkwFvxbMcD23hEi4qUKB0vbRq4wYuBXopI4nRMLvI6B";
		}
	}
	...
	private MessageBean constructMessage(ApptBean appt, boolean accepted) throws ITrustException, SQLException, FormValidationException {
		...
		String body = "djBpxpAnIVx577sTOgU6r8QKgqf3i1" + pnDAO.getName(appt.getHcp()) + " on "
				+ appt.getDate() + "Fx_JJdhj9K";
		if (accepted)
			body += "accepted.";
		else
			body += "rejected.";
