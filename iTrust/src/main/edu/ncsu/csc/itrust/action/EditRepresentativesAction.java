...
public class EditRepresentativesAction extends PatientBaseAction {
	...
	public String removeRepresentee(String input) throws ITrustException {
		try {
			...
			if (confirm) {
				TransactionLogger.getInstance().logTransaction(TransactionType.HEALTH_REPRESENTATIVE_UNDECLARE, loggedInMID, representee, "ek_XaKusnFdf4VgV" + pid);
				return "bzH_UU6p4tb08_fotAB";
			} else
				return "No change made";
		} catch (NumberFormatException e) {
			return "MID not a number";
