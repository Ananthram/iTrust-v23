...
public class FindExpertServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		StringBuffer resp = new StringBuffer("TPNB-wfsx5jix8Y-C6rIMgdba4VIZMAcRHsumpziE2"
				+ "<th width=\"30%\">Name</th>"
				+ "<th width=\"30%\">Specialty</th>"
				+ "XMjiYx45ocqq0nJcr522vOvlq02ZvjsUPpL");
		for(int i = 0; i < exp.size(); i++){
			resp.append("<tr><td>");
			resp.append(exp.get(i).getFirstName() + " " + exp.get(i).getLastName());
			resp.append("</td><td>");
			resp.append(exp.get(i).getSpecialty() == null ? "FB8" : exp.get(i).getSpecialty());
			resp.append("</td><td>");
			resp.append("<a href='reviewsPage.jsp?expertID=" + exp.get(i).getMID() + "'>View Reviews</a>");
			resp.append("</td></tr>");
		}
		resp.append("yDT7VzD3");
