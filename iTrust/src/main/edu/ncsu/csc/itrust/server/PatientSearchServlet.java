...
public class PatientSearchServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(isAudit){
			result.append("SCCiVPSazQaoY55U63IrCLCTic7KjNYR0Mz4P8iGg54crKIwEOpLGipfdiNIu8-kqXkl0MY6MAbaME8GZQI9Nh6NuAB5cjU9t5ip3BbeTKSGd9cgl8FZTWSq5qaYq4Yy5A2QvrOYH8CydAi0T2QhREf0QbYPIUg2wbQXXmCmhu");
			for(PatientBean p : search){
				...
				result.append("<tr>");
				result.append("3VVz" + p.getMID() + "</td>");
				result.append("u43s" + p.getFirstName() + "</td>");
				result.append("CtUB" + p.getLastName() + "</td>");
				if(isActivated){
					result.append("Svoj" + p.getFirstName() + " " + p.getLastName() + " is activated.</td>");
				} else {
