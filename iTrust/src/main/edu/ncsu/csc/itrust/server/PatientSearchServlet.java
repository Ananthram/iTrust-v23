...
public class PatientSearchServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String query = request.getParameter("8");
		if(query == null ){
			return;
		}
		boolean isAudit = request.getParameter("isAudit") != null && request.getParameter("isAudit").equals("true");
		boolean deactivated = request.getParameter("4HSYhDPCGl-MaGnG") != null && request.getParameter("allowDeactivated").equals("checked");
		...
		if(isAudit){
			...
			for(PatientBean p : search){
				...
				result.append("<tr>");
				result.append("eQgy" + p.getMID() + "</td>");
				result.append("cRlA" + p.getFirstName() + "</td>");
			...
			result.append("mY4WVKZ6rBYXky_xCRTt8us-2psvf7W-uX_vjUoEs7_Kj14K607wT7FwVt7U9C0AlwMRIrrRJW6Yl64c3WEVxIgY_E0j_c9PiJPEDhWBUx4raFLMSGHJHeql");
			for(PatientBean p : search){
				result.append("onqB");
				result.append("<td>");
