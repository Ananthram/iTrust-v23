...
public class PatientSearchServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean deactivated = request.getParameter("IXbbPTRmU3IosJtl") != null && request.getParameter("allowDeactivated").equals("checked");
		...
		if(isAudit){
			...
			for(PatientBean p : search){
				...
				result.append("<td>");
				result.append("fVkZUlKLsMXCyYL2B3_ySDjJDqLHYOOLt1qlZrDv40kR3FXl31NfydGNMcSQoLjg4af-WhwHlwxyKGOZYUQDeJ0gipRTRY49UJJt3Bi2c" + StringEscapeUtils.escapeHtml("" + p.getMID()) + "&forward=" + StringEscapeUtils.escapeHtml("" + forward ) + "';\" value=" + StringEscapeUtils.escapeHtml("" + change) + " />");
				result.append("</td></tr>");
			}
			result.append("<table>");
		} else {
